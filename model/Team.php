...
//            echo $e->getMessage();
    ...
    {
        return DB::selectMany("SELECT `members`.* FROM `teams`
             INNER JOIN `team_member` ON `team_member`.team_id = `teams`.id 
             INNER JOIN `members` ON `members`.id = team_member.member_id
             WHERE `teams`.id = :id ORDER BY members.name", ["id" => $this->id],Member::class);
    }

    public function captain()
    {
        return DB::selectOne("SELECT `members`.name FROM `team_member`
             INNER JOIN `members` ON `members`.id = team_member.member_id
             WHERE `team_member`.team_id = :id AND team_member.is_captain = 1", ["id" => $this->id],Member::class);
    }
